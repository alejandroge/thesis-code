...
for c in c_list:
    ...
    clf_log = LogisticRegression(C=c, penalty='l2', solver='liblinear')
    clf_log.fit(data_trans_train, labels_train)
    predicted_log = clf_log.predict(data_trans_val)
    acc = np.mean(labels_val == predicted_log)
...
start = time.time()
print('Predicting with C = '+str(best_c))
clf_log = LogisticRegression(C=best_c, penalty='l2', solver='liblinear')
clf_log.fit(data_trans_train, labels_train)
scores = []
for labels_,data_ in zip(labels_test, data_trans_test):
    if data_ != None:
        predicted_log = clf_log.predict(data_)
        scores.append(recall_at(np.array(labels_), predicted_log))
stop = time.time()
