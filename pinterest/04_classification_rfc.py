...
for c in c_list:
    ...
    predicted_rf = clf_rf.predict(data_trans_val)
    acc = np.mean(labels_val == predicted_rf)
...
clf_rf = RandomForestClassifier(n_estimators=best_c).fit(data_trans_train, labels_train)
scores = []
for labels_,data_ in zip(labels_test, data_trans_test):
    if data_ != None:
        predicted_rf = clf_rf.predict(data_)
        scores.append(recall_at(np.array(labels_), predicted_rf))
stop = time.time()
